<?php
	...
	if(isset($auth['hash']['preference']) and !$auth['hash']['preference'] == 'SHA512' and !$auth['hash']['preference'] == 'SHA256' and !$auth['hash']['preference'] == 'BLOWFISH')
	{ echo '<br/>Fatal error: Password hashing is user-defined, but the type of hashing has not been specified.'.PHP_EOL; die(); }

	if(file_exists($_['fs_root'].'includes/lib/auth/salt.php')) { require_once($_['fs_root'].'includes/lib/auth/salt.php'); }

	if(array_key_exists('salt',$auth)) {
		core::debug($_, '$salt obtained from salt.php');
	} else {
		$salt=substr(sha1(mt_rand()),0,22);
		if(file_put_contents($_['fs_root'].'includes/lib/auth/salt.php', '<?php'.PHP_EOL.'$auth[\'salt\']=\''.$salt.'\';')) {
